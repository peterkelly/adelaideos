...
/* These external variables correspond to symbols defined by the linker, which
   tell us where the kernel's code and data sections begin and end. We use these
   when setting up paging so that user-level processes are given access to the
   kernel's code, but not its data. */
...
pid_t start_process(void (*start_address)(void))
{
  ...
  identity_map(proc->pdir,0*MB,6*MB,PAGE_SUPERVISOR,PAGE_READ_ONLY);
  ...
  /* Temporary: allow process to write to video memory and kernel global
     variables (needed for write_to_screen until we get system calls) */
  identity_map(proc->pdir,VIDEO_MEMORY,VIDEO_MEMORY+PAGE_SIZE,
               PAGE_USER,PAGE_READ_WRITE);
  identity_map(proc->pdir,KERNEL_GLOBALS_START,KERNEL_GLOBALS_END,
               PAGE_USER,PAGE_READ_WRITE);
  #endif /* USE_SYSCALLS */
...
void kill_process(process *proc)
{
  ...
  /* If any of this process's children are still running, change their
     parent pid to -1, so they won't stick around for this process to call
     waitpid() on them (which will never happen, since it has exited) */
  pid_t child_pid;
  ...
  /* Only "free" the process if there is no parent to wait for it; otherwise,
     we need to keep what's left of this process around so that the parent can
     later call waitpid() to get its exit status */
  if (-1 == proc->parent_pid) {
    proc->exists = 0;
  }
  else {
    /* If the parent is currently suspended, waiting for this child process
       to die, then wake it up */
